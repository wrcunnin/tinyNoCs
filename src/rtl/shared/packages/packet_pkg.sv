...
`endif
  ...
  parameter int unsigned NumEndpoints = 16;
  parameter int unsigned EndpointIdBits = $clog2(NumEndpoints);

  typedef logic [AddressWidth-1:0] addr_t;

  typedef logic [PayloadWidth-1:0] payload_t;

  typedef logic [IdWidth-1:0] id_t;

  typedef logic [EndpointIdBits-1:0] endpoint_id_t;
  ...
  logic [0:NumEndpoints-1][0:1][EndpointIdBits-1:0] Mesh4x4Endpoints = {
    // {x_idx, y_idx}
  ...
  logic [0:NumEndpoints-1][0:1][EndpointIdBits-1:0] Mesh5x3Endpoints = {
    // {x_idx, y_idx}
...
`endif
  ...
  parameter int unsigned NetPacketBits = $bits(net_packet_t);
...
assign endpoint_``id``_resp_comp_packet = resp_comp_packet[id];
  ...
  `define CONNECT_MESH_XBARS(xid, yid, net_name, in_direction,
                             out_direction) \
...
torus_xbar #( \
  ...
  `define CONNECT_TORUS_XBARS(xid, yid, net_name, in_direction,
                              out_direction) \
